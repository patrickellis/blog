...
func createSnippet(content, query string) template.HTML {
	...
	if idx == -1 {
		if len(content) > 150 {return template.HTML(template.HTMLEscapeString(content[:150]) + "...")}
		return template.HTML(template.HTMLEscapeString(content))
	}
	start := idx - 75; if start < 0 {start = 0}
	end := idx + len(query) + 75; if end > len(content) {end = len(content)}
...
func handleSearchPopup(w http.ResponseWriter, r *http.Request) {
	query := r.FormValue("q")
	if len(query) < 2 {
		fmt.Fprintf(w, "<p>Please enter at least 2 characters.</p>"); return
	}
	var results []SearchResult
	cache.mu.RLock(); posts := cache.posts; cache.mu.RUnlock()
	for _, post := range posts {
		...
		if titleMatch || contentMatch {
			results = append(results, SearchResult{Title: post.Title, Slug: post.Slug, Snippet: createSnippet(post.RawContent, query)})
		}
	}
